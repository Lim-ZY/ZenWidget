...
interface ZenDao {
    ...
    fun getItemsForRepo(repoType: RepoType): Flow<List<RepoItem>>

    @Insert
    suspend fun insertItem(item: RepoItem)

    @Update
    suspend fun updateItem(item: RepoItem)
    ...
    suspend fun getItemAt(repoType: RepoType, index: Int): RepoItem?

    @Query("SELECT COUNT(*) FROM widget_items WHERE repoType = :repoType")
    suspend fun getCount(repoType: RepoType): Int
}